...
HAL_StatusTypeDef as5600_get_angle(encoder_typedef* const enc_handle, state_typedef* state_handle) {
	uint8_t data[2] = {0};
    static int temp;
	...
	state_handle->actual_position = ((data[0] << 8) | data[1]);
    state_handle->iqPos = convert_pulse_to_position(state_handle->actual_position);
    state_handle->iqPosElec = compute_position_elec(state_handle->iqPos);
    temp = (state_handle->actual_position - state_handle->actual_position_prev) / _IQtoF(oConfig->CONST_POSITION_SAMP_TIME);
    
    if(temp > _IQ15int(oConfig->CONST_MAX_VELOCITY))
    {
        temp -= _IQ15int(oConfig->CONST_ENC_RESOLUTION) / _IQtoF(oConfig->CONST_POSITION_SAMP_TIME);
    }
    if(temp < -_IQ15int(oConfig->CONST_MAX_VELOCITY))
    {
        temp += _IQ15int(oConfig->CONST_ENC_RESOLUTION) / _IQtoF(oConfig->CONST_POSITION_SAMP_TIME);
    }
    state_handle->actual_velocity = temp + BIT_15;
    
    
    state_handle->iqVel = convert_pulse_to_velocity(state_handle->actual_velocity);
    state_handle->iqVel = filter_update(oFilterVelocity, state_handle->iqVel);
    state_handle->iqPosPrev = state_handle->iqPos;
    state_handle->actual_position_prev = state_handle->actual_position;
	return status;
}
